...
| Time | Session | Topics |
|---|---|---|
| 10:30 – 11:30 | **Secure Coding Practices** | OWASP Top 10, injection, broken auth, code-level controls on LMS |
| 11:30 – 12:15 | **SAST Concepts + Tools** | Static analysis, SonarQube, taint analysis, LMS scan walkthrough |
| 12:15 – 12:30 | Break | — |
| 12:30 – 1:30 | **Lab 1: SAST Scan + Fix** | Hands-on SonarQube scan of LMS, triage, remediation |
| 1:30 – 2:15 | **DAST Concepts + Tools** | Dynamic testing, OWASP ZAP, scanning running apps |
| 2:15 – 3:00 | Lunch | — |
| 3:00 – 4:00 | **Lab 2: DAST Scan** | ZAP active scan of LMS, vulnerability analysis |
...
Secure coding is the practice of writing software that is resilient to attack from the moment it is written — not as an afterthought. For a banking application like LMS, a single exploitable flaw in a REST endpoint can expose CIBIL scores and loan data, enable unauthorized access to application records, or allow an attacker to pivot deeper into the bank's network. This module covers the most common vulnerability classes and how to eliminate them at the code level.

## 1.1 Why Secure Coding Matters in Banking

Banks are among the most targeted organisations in the world. RBI's IT Framework for Banks (2011, updated 2023) and CERT-In directives mandate that development teams follow secure coding standards. The cost of a post-deployment fix is 6–100x higher than catching the same issue at code review. The LMS application — though built as a training vehicle — uses the same patterns, frameworks, and data types (PII: CIBIL score, PAN, income) found in real banking systems, making it an ideal sandbox to learn these concepts without real risk.

## 1.2 The OWASP Top 10 — Banking Context

The Open Web Application Security Project (OWASP) publishes a Top 10 list of the most critical web application security risks. Every item on this list has caused real incidents at financial institutions. We cover each one with an LMS-specific example.
...
**LMS Example** — CIBIL score and monthly income are PII. Only a Branch Manager or assigned Loan Officer should see them. A broken access control vulnerability would allow any authenticated user to call:

```
GET /api/v1/applications/42  →  returns { cibilScore: 720, monthlyIncome: 85000 } to ANY authenticated user
```

The fix is Role-Based Access Control (RBAC) enforced at the method level using Spring Security:

```java
// Vulnerable — no access control on PII fields
@GetMapping("/{id}")
...
// Secure — PII masked unless requester is MANAGER
@GetMapping("/{id}")
@PreAuthorize("hasAnyRole('MANAGER','OFFICER')")
public ResponseEntity<LoanApplicationDTO> getApplication(@PathVariable Long id,
        Authentication auth) {
    LoanApplicationDTO dto = applicationService.findById(id);
    if (!securityService.isManager(auth)) {
        dto.setCibilScore(null);
        dto.setMonthlyIncome(null);
        dto.setPanNumber(maskPan(dto.getPanNumber())); // show last 4 only
    }
...
Cryptographic failures (formerly 'Sensitive Data Exposure') occur when sensitive data is transmitted or stored without adequate encryption. In LMS, CIBIL score, PAN number, and monthly income are PII and must never appear in plain text in logs, error messages, or unencrypted storage.

- Always use HTTPS (TLS 1.2+). Spring Boot with `spring.ssl.*` properties enforces this.
- Never log sensitive fields. Use `@JsonIgnore` on cibilScore and monthlyIncome in log-facing DTOs.
...
// Masking in logs — never log the raw LoanApplicationDTO
@Override
public String toString() {
    return "LoanApplication{id=" + id + ", pan=[REDACTED], cibilScore=[REDACTED], income=[REDACTED]}";
}
```
...
LMS uses Spring Data JPA, which uses parameterized queries by default. However, custom JPQL or native queries can introduce injection if written carelessly:

```java
// VULNERABLE — string concatenation in JPQL
@Query("SELECT e FROM LoanApplication a WHERE a.branch.name = '" + branchName + "'")
List<LoanApplication> findByBranchNameUnsafe(String branchName);

// SAFE — parameterized JPQL
@Query("SELECT e FROM LoanApplication a WHERE a.branch.name = :branchName")
List<LoanApplication> findByBranchName(@Param("branchName") String branchName);

// SAFEST — Spring Data derived query (no SQL string at all)
List<LoanApplication> findByBranchName(String branchName);
```

> **Banking-Specific Risk:** SQL injection on an application search endpoint could allow an attacker to extract ALL loan application records including PAN and CIBIL data with a single crafted query: `?branch=' OR '1'='1`. In a real bank system, this means mass PII exposure — PAN numbers, CIBIL scores, and income data triggering RBI data breach notification requirements.

---

### A04:2021 — Insecure Design

Insecure design means architectural or design-level flaws that cannot be fixed by implementation alone. In LMS, the business rule "an application cannot move directly from SUBMITTED to APPROVED — it must pass through UNDER_REVIEW first" is a design control. If it is not enforced at the service layer, no amount of controller-level validation will prevent it.

```java
// Service layer enforcing loan application lifecycle state machine
...
```properties
# application.properties — LMS secure configuration
...
```bash
# Scan LMS for known CVEs — run from project root
dependency-check.sh --project LMS --scan . --format HTML
...
LMS uses JWT (JSON Web Token) for stateless authentication. Improper JWT implementation is a critical vulnerability class — attackers can forge tokens, bypass expiry, or exploit weak signing secrets.
...
Banks are required by RBI to maintain audit logs for access to sensitive data. LMS uses Spring Actuator and should be wired to a SIEM. At minimum, log all authentication events, all access to CIBIL scores and PAN numbers, and all failed authorization attempts.

```java
// Audit logging — log every PII field access using Spring AOP
@Aspect
@Component
public class SensitiveFieldAccessAuditAspect {

    private static final Logger audit = LoggerFactory.getLogger("AUDIT");

    @AfterReturning(pointcut = "execution(* *.getApplication(..))",
                    returning = "result")
...
LMS uses Bean Validation (Jakarta Validation API) for input validation. Every field that accepts user input must be validated for type, length, format, and business rules before it reaches the service layer.

```java
// LMS LoanApplicationRequest DTO — comprehensive validation
public class LoanApplicationRequest {
    ...
    @Pattern(regexp = "^[A-Z]{5}[0-9]{4}[A-Z]{1}$",
             message = "Must be a valid PAN number format")
...
Error messages are intelligence for attackers. A stack trace revealing package names, database types, or query structures gives an attacker a significant advantage. LMS must use a global exception handler that returns safe, generic messages to the client while logging the full detail internally.

```java
// LMS Global Exception Handler
...
| Severity | Meaning | Example in LMS |
|---|---|---|
...
## 2.4 Configuring SonarQube for LMS

```xml
<!-- LMS pom.xml — SonarQube plugin configuration (already present) -->
<properties>
...
> **Lab Preview:** In Lab 1 (12:30 – 1:30), you will run a real SonarQube scan on LMS and triage the findings. You will fix at least two Critical issues — a hardcoded credential and a missing authorization check — and re-run the scan to confirm the Quality Gate passes.
...
```bash
# Scan LMS with OWASP Top 10 rules
...
```yaml
# Custom rule — detect hardcoded passwords in LMS
...
# Lab 1: SAST Scan + Fix
`12:30 – 1:30` · Hands-on SonarQube scan of LMS · Triage findings · Remediate two Critical issues

> **Lab Objective:** Run a SonarQube scan on the LMS project, understand the findings, fix two Critical security issues (hardcoded secret + missing authorization), re-scan, and verify the Quality Gate passes.

## Step 1 — Open the Project

1. Open VS Code.
2. **File > Open Folder** > select the `lms/` folder on your Desktop.
...
At the end, the console prints a URL: `ANALYSIS SUCCESSFUL, you can find the results at: http://SONAR_IP:9000/dashboard?id=sbi-lms`
...
## Step 6 — Fix Issue 2: Missing Authorization on PII Endpoint

Find the application GET endpoint in `ApplicationController.java` and apply the fix from Module 1:
...
public ResponseEntity<LoanApplicationDTO> getApplication(@PathVariable Long id,
    ...
    if (!securityService.isManager(auth)) {
        dto.setCibilScore(null);
        dto.setMonthlyIncome(null);
        dto.setPanNumber(maskPan(dto.getPanNumber()));
    }
    return ResponseEntity.ok(dto);
}
```
...
## 3.4 Scanning LMS with ZAP

LMS exposes a REST API — we use the OpenAPI (Swagger) specification that SpringDoc generates to import all endpoints into ZAP automatically.
...
# Step 5 — Active Scan
# Right-click the LMS context > Attack > Active Scan
```

## 3.5 Interpreting DAST Findings for LMS

| Finding | Affected LMS Endpoint | Remediation |
|---|---|---|
...
| Verbose error messages in 500 responses | `/api/v1/applications` | Use GlobalExceptionHandler (Module 1 — already fixed) |
| JWT token in URL parameter | `/api/v1/auth/**` | Always send JWT in Authorization header, never in URL |
| CORS: `Access-Control-Allow-Origin: *` | All endpoints | Restrict: `allowedOrigins("https://lms.sbi.internal")` |

## 3.6 Adding Security Headers to LMS

```java
// LMS SecurityConfig.java — add security response headers
@Bean
...
CorsConfigurationSource corsConfig() {
    ...
    cfg.setAllowedOrigins(List.of("https://lms.sbi.internal"));
...
`3:00 – 4:00` · OWASP ZAP active scan of running LMS · Analyse alerts · Add security headers

> **Lab Objective:** Start LMS, import its OpenAPI spec into ZAP, run an active scan, analyse the findings, and apply the CORS + security header fixes from Module 3.
...
3. Click Import. ZAP discovers all LMS endpoints automatically.
...
private static final String DB_PASSWORD = "SBI_LMS_DB_2024!";

// Pattern 2 — In application.properties committed to git
// spring.datasource.password=SBI_LMS_DB_2024!
```

```yaml
# Pattern 3 — In a Docker Compose file committed to git
environment:
  MYSQL_ROOT_PASSWORD: SBI_LMS_DB_2024!
```
...
```bash
# Scan the LMS repository for existing secrets
detect-secrets scan ./lms > .secrets.baseline
...
spring.datasource.url=jdbc:mysql://${DB_HOST:localhost}:3306/${DB_NAME:lms}
...
- **Dynamic secrets:** Vault generates short-lived, just-in-time database credentials. LMS's MySQL password rotates automatically — no static password to steal.
...
# Retrieve
vault kv get -field=db_password secret/lms
```
...
spring.cloud.vault.kv.application-name=lms
...
## 5.2 GitHub Actions Pipeline for LMS

```yaml
# .github/workflows/devsecops.yml
name: LMS DevSecOps Pipeline
...
jobs:
  security-scan:
    ...
    - name: Build Docker image
      run: docker build -t lms:${{ github.sha }} .
    ...
    - name: Start LMS for DAST
      run: |
        docker run -d --name lms-test \
          -p 8080:8080 \
...
| Module | Key Takeaway | LMS Application |
|---|---|---|
| Secure Coding | OWASP Top 10 in Java Spring Boot; validation; safe error handling | Added @PreAuthorize on CIBIL/income fields; fixed exception handler |
| SAST | SonarQube taint analysis; Quality Gates; IDE integration | Scanned LMS; fixed hardcoded JWT secret; fixed missing auth |
| DAST | ZAP active scan; API scanning via OpenAPI; alert triage | Scanned running LMS; added security headers; fixed CORS |
...
| CI/CD | Pipeline gates at pre-commit, build, and deploy stages | Reviewed full GitHub Actions pipeline for LMS |
